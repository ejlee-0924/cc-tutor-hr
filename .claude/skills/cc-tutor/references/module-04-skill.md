...
{name} {title}님이 특정 분야 교육이 필요할 때 검증된 인재 풀에서 바로 섭외하시죠? **Skill도 그런 겁니다** — 검증된 전문 역량을 필요할 때 "/스킬명" 한 줄로 바로 꺼내 씁니다.

매번 긴 프롬프트를 작성하는 대신, 누군가 이미 만들어둔 전문 스킬을 설치해서 바로 쓸 수 있습니다.

**Before:** "JD 작성해줘. 근데 이런 형식으로, 이런 톤으로, 이런 구조로..." 매번 길게 설명
**After:** `/proposal` 한 줄이면 전문가급 JD 초안 완성

## EXPLAIN

### Skill이란?

> 📖 glossary 참조: Skill = "검증된 인재 풀"

특정 업무에 특화된 전문 역량을 패키징한 것입니다. Claude Code에 설치하면 `/스킬명`으로 바로 호출할 수 있습니다.

| 항목 | 내용 |
|------|------|
| 비유 | 검증된 인재 풀 — 특정 분야 전문가를 필요할 때 바로 투입 |
...
| 스킬 | 기능 | 활용 예시 |
|------|------|----------|
| **PowerPoint (pptx)** | 프레젠테이션 생성·편집 | "신입사원 온보딩 발표자료 만들어줘" |
| **Excel (xlsx)** | 스프레드시트·데이터 분석·차트 | "채용 현황 대시보드 만들어줘" |
| **Word (docx)** | 문서 생성·편집·포맷팅 | "취업규칙 개정안 문서 만들어줘" |
| **PDF (pdf)** | PDF 문서·보고서 생성 | "교육 수료증 PDF 만들어줘" |
...
📋 아래를 복사해서 입력하세요:
    ...
    개발팀에서 시니어 백엔드 개발자 채용을 요청했어.
    필수 역량: Kotlin/Java, 대규모 트래픽 경험
    경력: 7년 이상
    연봉 밴드: 협의

    이 정보를 바탕으로 채용 공고 초안을 작성해줘.
...
>   - Clarify: 모호한 현업 요구사항을 구조화 (모듈 7에서 배웁니다)
> - **모듈 8에서 우리 팀만의 스킬을 직접 만들 겁니다** — 제안서 자동화, 후보자 소싱 등
