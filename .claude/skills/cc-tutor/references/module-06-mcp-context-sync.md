...
{name} {title}님, 제안서 쓸 때 현업 부서 정보가 어디에 흩어져 있나요?

슬랙에서 현업 요청을 확인하고, 노션에서 이전 JD를 찾고, 메일에서 일정을 확인하고... **MCP는 이 모든 곳을 Claude가 직접 연결해서 정보를 가져오게 하는 파이프**입니다.

**Before:** 슬랙·노션·메일 3개 채널을 뒤져서 고객 정보 취합 (30분+)
**After:** "오픈 포지션 관련 정보 모아줘" 한 마디로 자동 수집
...
| MCP 서버 | B2B 비유 | 활용 예시 |
|----------|---------|----------|
| Slack | 팀 대화방 연결 | "현업 부서 채널에서 요청사항 가져와" |
| Notion | 사내 위키 연결 | "이전 제안서 찾아줘" |
| Gmail | 메일함 연결 | "고객 메일에서 일정 확인해줘" |
| Calendar | 일정표 연결 | "다음 주 현업 미팅 확인해줘" |
...
여러 MCP 소스에서 가져온 정보를 **하나의 문서로 정리**하는 것입니다. 현업 부서별로 흩어진 정보를 JD 작성에 바로 쓸 수 있게 모아줍니다.
...
📋 아래를 복사해서 입력하세요:
    ...
    시니어 PM 채용 프로젝트 관련 정보를 수집하는 시나리오를 보여줘.
...
📋 아래를 복사해서 입력하세요:
    ...
    JD 작성에 바로 쓸 수 있는 "포지션 브리핑 문서" 형태로 정리해줘.
...
>
> - **팀 전체가 같은 MCP 설정**을 쓰면, 누가 JD를 써도 같은 정보 소스에 접근합니다
> - 현업 부서별 Context Sync를 자동화하면 **제안서 준비 시간이 절반**으로 줄어듭니다
